...
- [tests/test_tools/test_anr_parser.py](file://tests/test_tools/test_anr_parser.py)
- [tests/test_tools/test_je_parser.py](file://tests/test_tools/test_je_parser.py)
- [tests/test_tools/test_ne_parser.py](file://tests/test_tools/test_ne_parser.py)
</cite>

## 更新摘要
**变更内容**   
- 新增测试基础设施章节，详细介绍ANR、Java异常、Native崩溃解析器测试和Orchestrator测试
- 更新测试策略与编写方法部分，包含新的测试框架和最佳实践
- 增强故障排查指南，添加测试相关的调试技巧
- 完善新功能开发模板，包含测试用例编写指导
...
B6 --> B6b["memory.py"]
B6 --> B6c["reflector.py"]
E --> E1["test_core<br/>核心组件测试"]
E --> E2["test_tools<br/>工具层测试"]
E --> E3["test_agents<br/>Agent测试"]
E --> E4["test_knowledge<br/>知识层测试"]
E1 --> E1a["test_orchestrator.py"]
E2 --> E2a["test_anr_parser.py"]
E2 --> E2b["test_je_parser.py"]
E2 --> E2c["test_ne_parser.py"]
```
...
### 测试架构概览
Jirin采用分层测试策略，涵盖单元测试、集成测试和端到端测试。测试框架基于pytest，支持异步测试、参数化测试和模拟对象。
...
### ANR解析器测试
ANR解析器测试专注于验证Android应用无响应问题的日志解析准确性。测试用例涵盖：
- 标准ANR堆栈解析
- Binder超时检测
- 主线程阻塞分析
- 输入分发超时识别
...
### 测试最佳实践
- **测试数据管理**：使用fixtures存放样例日志和配置，保持数据最小化和可重复性
- **断言策略**：明确断言目标（输入/输出/副作用），设置覆盖率阈值
- **模拟对象**：使用mock隔离外部依赖，提高测试稳定性
- **异步测试**：正确处理异步操作和并发场景
- **测试命名**：遵循清晰的测试命名约定，便于理解和维护
...
## 结论
Jirin以模块化与可扩展为核心设计，通过清晰的层次划分与统一接口，实现了跨场景的分析能力。新增的测试基础设施确保了代码质量和稳定性。遵循本指南中的规范与实践，可有效提升开发效率与交付质量。
